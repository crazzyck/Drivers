...
class MCP47FEdef:
    ...
    # memory map address
    CHANNELS = [id for id in range(7)]
    POWER_DOWN_REG = 0x09
    GAIN_SETUP_REGISTER = 0x0A
    WIPERLOCK_REG = 0x0B


class MCP47FEException(Exception):
...
class MCP47FE(object):
    ...
    def set_all_gain(self, gain):
        '''
        MCP47FE set all channel gain

        Args:
            gain:       int, [1, 2]
        Examples:
            mcp47FE.set_all_gain(1)

        '''
        assert gain in [1, 2]
        if gain == 1:
            self._gain_register = MCP47FEdef.REG_GAIN_1 & 0x00  # 0b00000000
        else:
            self._gain_register = MCP47FEdef.REG_GAIN_2 | 0xFF  # 0b11111111
        # self._gain_register = (MCP47FEdef.REG_GAIN_1 if gain == 1 else MCP47FEdef.REG_GAIN_2)
        self._write_register(MCP47FEdef.WRITE_REG, MCP47FEdef.GAIN_SETUP_REGISTER, (self._gain_register >> 8) & 0xFF,
                             self._gain_register & 0xFF)

    def set_single_gain(self, channel, gain):
        '''
        MCP47FE set specific channel gain

        Args:
            channel:    int, [0~7], channel id.
            gain:       int, [1, 2]
        Examples:
            mcp47FE.set_single_gain(0, 1)

        '''
        assert channel in MCP47FEdef.CHANNELS
        assert gain in [1, 2]
        if gain == 1:
            self._gain_register = MCP47FEdef.REG_GAIN_1
        else:
            self._gain_register = MCP47FEdef.REG_GAIN_2
        self._gain_register = self._gain_register << channel
        self._write_register(MCP47FEdef.WRITE_REG, MCP47FEdef.GAIN_SETUP_REGISTER,
                             (self._gain_register >> 8) & 0xFF, self._gain_register & 0xFF)

    def output_volt_dc(self, channel, volt):
        '''
        MCP47FE output dc voltage
        ...
        Examples:
            mcp47FE.output_volt_dc(0, 1000)

        '''
        assert channel in MCP47FEdef.CHANNELS
        assert isinstance(volt, (int, float)) and 0 <= volt <= self._mvref
        # mode = MCP47FEdef.POWER_MODE_NORMAL
        # Output Voltage resolution is 8-bit
        dac_code = int(volt * float(256) / self._mvref * self._dac_gain)
        if dac_code == 256:
            dac_code = 255
        high_byte = (dac_code >> 8) & 0xFF
        low_byte = dac_code & 0xFF
        self._write_register(MCP47FEdef.WRITE_REG, channel, high_byte, low_byte)
